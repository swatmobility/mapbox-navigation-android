...
import com.mapbox.navigation.dropin.util.weakReference
...
internal class MapLayoutCoordinator(
    ...
    private val cameraStateCache = navigationViewContext.cameraStateCache
    private var mapboxMap: MapboxMap? by weakReference()

    override fun onDetached(mapboxNavigation: MapboxNavigation) {
        super.onDetached(mapboxNavigation)
        mapStyleLoader.mapboxMap = null
        mapboxMap?.also { cameraStateCache.saveCameraState(it) }
    }
    ...
    override fun MapboxNavigation.flowViewBinders(): Flow<Binder<ViewGroup>> {
        return navigationViewContext.mapView
            ...
            .map { mapView ->
                mapView.getMapboxMap().also { map ->
                    mapboxMap = map
                    cameraStateCache.restoreCameraState(map)
                }
